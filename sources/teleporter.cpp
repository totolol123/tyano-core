...
Tile* Teleporter::getAvailableDestinationTile(const Thing& thing) const {
	...
	for (bool ignoresBlockingCreatures = false; !ignoresBlockingCreatures; ignoresBlockingCreatures = true) {
		...
		if (tile->__queryAdd(INDEX_WHEREEVER, &thing, 1, directFlags) == RET_NOERROR) {
			return tile;
		}
		...
		for (uint32_t distance = 1; distance <= MAXIMUM_DISTANCE; ++distance) {
			...
			if (!alternativeTiles.empty()) {
				...
				for (auto alternativeTile : alternativeTiles) {
					if (tile->__queryAdd(INDEX_WHEREEVER, &thing, 1, indirectFlags) != RET_NOERROR) {
						continue;
					}
