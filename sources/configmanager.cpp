...
LOGGER_DEFINITION(ConfigManager);
...
ConfigManager::ConfigManager()
{
	...
	m_confString[CONFIG_FILE] = "config.lua";
...
bool ConfigManager::load()
{
	...
	m_confNumber[MAXIMUM_DOOR_LEVEL] = getGlobalNumber("maximumDoorLevel", 9999999);
...
const std::string& ConfigManager::getString(uint32_t _what) const
{
	...
	if(!m_startup)
		LOGe("[ConfigManager::getString] " << _what);
...
bool ConfigManager::getBool(uint32_t _what) const
{
	...
	if(!m_startup)
		LOGe("[ConfigManager::getBool] " << _what);
...
int32_t ConfigManager::getNumber(uint32_t _what) const
{
	...
	if(!m_startup)
		LOGe("[ConfigManager::getNumber] " << _what);
...
double ConfigManager::getDouble(uint32_t _what) const
{
	...
	if(!m_startup)
		LOGe("[ConfigManager::getDouble] " << _what);
...
bool ConfigManager::setString(uint32_t _what, const std::string& _value)
{
	...
	LOGe("[ConfigManager::setString] " << _what);
	return false;
}
...
bool ConfigManager::setNumber(uint32_t _what, int32_t _value)
{
	...
	LOGe("[ConfigManager::setNumber] " << _what);
	return false;
}


void ConfigManager::getValue(const std::string& key, lua_State* luaState) {LuaScriptInterface::getValue(key, L, luaState);}
...
bool ConfigManager::getGlobalBool(const std::string& _identifier, bool _default)
{
	return LuaScriptInterface::getGlobalBool(L, _identifier, _default);
}
int32_t ConfigManager::getGlobalNumber(const std::string& _identifier, const int32_t _default)
{
	return LuaScriptInterface::getGlobalNumber(L, _identifier, _default);
}
double ConfigManager::getGlobalDouble(const std::string& _identifier, const double _default)
{
