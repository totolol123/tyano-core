...
class TileItemVector
{
	public:
		...
		Item* at(size_t _pos) {return items.at(_pos).get();}
		Item* at(size_t _pos) const {return items.at(_pos).get();}
		Item* back() {return items.back().get();}
		const Item* back() const {return items.back().get();}
...
public:
	...
	public:
		...
		virtual uint32_t __getItemTypeCount(uint16_t itemId, int32_t subType = -1, bool itemCount = true, bool includeSlots = true) const;
