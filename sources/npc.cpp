	  ...
	  m_npcEventHandler(nullptr)
{
	...
	npcCount++;
#endif
	m_filename = getFilePath(FileType::OTHER, "npc/" + _name + ".xml");
	if(!fileExists(m_filename.c_str()))
	{
		std::string tmp = getFilePath(FileType::MOD, "npc/" + _name + ".xml");
...
bool Npc::load()
{
	...
	if(!m_interface)
	{
		...
		m_interface->loadNpcLib(getFilePath(FileType::OTHER, "npc/lib/npc.lua"));
	}
...
bool Npc::loadFromXml(const std::string& filename)
{
	...
	if(!doc)
	{
		LOGe("[Npc::loadFromXml] Cannot load npc file (" << filename << "):" << getLastXMLError());
		return false;
	}
	...
	{
		LOGe("[Npc::loadFromXml] Malformed npc file (" << filename << ").");
		xmlFreeDoc(doc);
		return false;
	}
	...
	{
		LOGw("[Npc::Npc] NPC Name: " << name << " - autowalk has been deprecated, use walkinterval.");
		walkTicks = 2000;
	}
	...
	if(scriptfile.find("/") == std::string::npos)
		scriptfile = getFilePath(FileType::OTHER, "npc/scripts/" + scriptfile);
...
uint32_t Npc::loadParams(xmlNodePtr node)
{
	...
	{
		StringVector paramList = explodeString(strValue, ";");
		for(StringVector::iterator it = paramList.begin(); it != paramList.end(); ++it)
		{
			...
			if(tmpParam == "male")
				...
				LOGw("[Npc::loadParams] NPC Name: " << name << " - Unknown param " << (*it));
		}
	}
...
ResponseList Npc::loadInteraction(xmlNodePtr node)
{
	...
	while(node)
	{
		if(xmlStrcmp(node->name, (const xmlChar*)"include") == 0)
		{
			...
			{
				if(xmlDocPtr doc = xmlParseFile(getFilePath(FileType::OTHER, "npc/lib/" + strValue).c_str()))
				{
					...
					if(!xmlStrcmp(root->name,(const xmlChar*)"interaction"))
					{
						...
						LOGw("[Npc::loadInteraction] Malformed interaction file (" << strValue << ").");

					xmlFreeDoc(doc);
				}
				else
				{
					LOGe("[Npc::loadInteraction] Cannot load interaction file (" << strValue << "): " << getLastXMLError());
				}
			}
		}
				...
				if(it == itemListMap.end())
				{
					...
					while(tmpNode)
					{
						if(!xmlStrcmp(tmpNode->name, (const xmlChar*)"item"))
						{
							...
							{
								LOGw("[Npc::loadInteraction] NPC Name: " << name << " - Missing list item itemId");
							...
							ItemKindPC kind = server.items()[li.itemId];
							if (!kind) {
								LOGw("[Npc::loadInteraction] NPC Name: " << name << " - There is no item " << li.itemId << ".");
								tmpNode = tmpNode->next;
								continue;
							}
							...
							{
								LOGw("[Npc::loadInteraction] NPC Name: " << name << " - Missing list item keywords");
								tmpNode = tmpNode->next;
								continue;
							}
							...
							else
                            {
								if(kind->stackable)
									li.subType = 1;
								else if(kind->isFluidContainer() || kind->isSplash())
									li.subType = 0;
                            }
					...
					LOGw("[Npc::loadInteraction] NPC Name: " << name << " - Duplicate listId found: " << strValue);
			}
		}
						...
						if(!xmlStrcmp(listNode->name, (const xmlChar*)"text"))
						{
							if(readXMLContentString(listNode, strValue))
							{
								...
								if(it != itemListMap.end())
									...
									LOGw("[Npc::loadInteraction] NPC Name: " << name << " - Could not find a list id called: " << strValue);
							}
						}
							...
							if(readXMLString(subNode, "name", strValue))
							{
								...
								else if(tmpStrValue == "spell")
								{
									if(readXMLString(subNode, "value", strValue))
									{
										...
										action.strValue = strValue;
										if(strValue != "|SPELL|" && !server.spells().getInstantSpellByName(strValue))
											LOGw("[Npc::loadInteraction] NPC Name: " << name << " - Could not find an instant spell called: " << strValue);
									}
								}
								...
								else if(tmpStrValue == "teleport")
								{
									...
									LOGw("[Npc::loadInteraction] Unknown action " << strValue);
							}
					...
					if(hasListKeyword && !prop.itemList.empty())
					{
						for(std::list<ListItem>::iterator it = prop.itemList.begin(); it != prop.itemList.end(); ++it)
						{
								...
								iit != listItemProp.inputList.end(); ++iit)
							{
								...
								if(it->name.empty())
								{
									ItemKindPC kind = server.items()[it->itemId];
									if(kind)
										action.strValue = kind->article + " " + kind->name;
								}
								else
								...
								if(it->pluralName.empty())
								{
									ItemKindPC kind = server.items()[it->itemId];
									if(kind)
										action.strValue = kind->pluralName;
								}
								else
...
void Npc::onThink(uint32_t interval)
{
	...
	const SpectatorList& tmpList = server.game().getSpectators(getPosition());
	if(tmpList.size()) //loop only if there's at least one spectator
	{
		for(SpectatorList::const_iterator it = tmpList.begin(); it != tmpList.end(); ++it)
		{
		...
		for(ActionList::const_iterator it = response->getFirstAction(); it != response->getEndAction(); ++it)
		{
			switch(it->actionType)
			{
				...
				case ACTION_SETEFFECT:
					server.game().addMagicEffect(player->getPosition(), it->intValue);
					break;
				...
				case ACTION_SETTELEPORT:
				{
					...
					server.game().internalTeleport(player, teleportTo, true);
					break;
				}
				...
				case ACTION_SETLEVEL:
				{
					...
					{
						npcState->level = -1;
						if(InstantSpellP spell = server.spells().getInstantSpellByName(npcState->spellName))
				...
				{
					npcState->spellName = "";
					if(server.spells().getInstantSpellByName(it->strValue))
				...
				{
					ItemKindPC kind = server.items()[npcState->itemId];
					if(kind)
					{
						...
						int32_t itemCount = player->__getItemTypeCount(kind->id, subType);
						if(itemCount >= npcState->amount)
						{
							server.game().removeItemOfType(player, kind->id, npcState->amount, subType);
				...
				case ACTION_BUYITEM:
				{
					...
					if(kind)
					{
						...
						if(server.game().getMoney(player) >= moneyCount)
						{
							...
							if(kind->stackable)
							{
								while(amount > 0)
								{
									...
									boost::intrusive_ptr<Item> item = Item::CreateItem(kind->id, stack);
									if(server.game().internalPlayerAddItem(this, player, item.get()) != RET_NOERROR)
									{
							...
							else
							{
								...
								{
									boost::intrusive_ptr<Item> item = Item::CreateItem(kind->id, subType);
									if(server.game().internalPlayerAddItem(this, player, item.get()) != RET_NOERROR)
									{
										amount = i + 1;
										break;
							...
							server.game().removeMoney(player, moneyCount);
						}
					}
					break;
				}
				...
				case ACTION_TAKEITEM:
				{
					...
					if(kind)
					{
						...
						if(itemCount >= npcState->amount)
							server.game().removeItemOfType(player, itemId, npcState->amount, subType);
					}
					break;
				}
				...
				case ACTION_GIVEITEM:
				{
					...
					if(kind)
					{
						...
						{
							boost::intrusive_ptr<Item> item = Item::CreateItem(kind->id, subType);
							server.game().internalPlayerAddItem(this, player, item.get());
						}
					}
					break;
				}
				...
				case ACTION_GIVEMONEY:
				{
					...
					server.game().addMoney(player, moneyCount);
					break;
				}
		...
		if(response->getResponseType() == RESPONSE_DEFAULT)
		{
			...
			if(!responseString.empty())
			{
				if(!response->publicize())
					...
					doSay(responseString, SPEAK_SAY, nullptr);
			}
		}
		else
		{
				...
				if(m_interface->reserveEnv())
				{
					...
					for(ActionList::const_iterator it = response->getFirstAction(); it != response->getEndAction(); ++it)
					{
						if(it->actionType == ACTION_SCRIPTPARAM)
						{
							...
							{
								LOGw("[Npc::executeResponse] Unknown script param: " << it->strValue);
								break;
							}
...
void Npc::doSay(const std::string& text, SpeakClasses type, Player* player)
{
	if(!player)
	{
		...
		server.game().internalCreatureSay(this, type, tmp, false);
	}
	else
	{
...
{
	server.game().internalMoveCreature(this, dir);
}
	...
	NpcState* npcState, const std::string& text, bool exactMatch /*= false*/)
{
	...
	StringVector wordList = explodeString(textString, " ");
	...
	for(ResponseList::const_iterator it = list.begin(); it != list.end(); ++it)
	{
		...
		if((*it)->getParams() != RESPOND_DEFAULT)
		{
			...
			{
				if((signed)server.game().getMoney(player) >= (npcState->price * npcState->amount))
					continue;
...
uint32_t Npc::getMatchCount(NpcResponse* response, StringVector wordList,
	bool exactMatch, int32_t& matchAllCount, int32_t& totalKeywordCount)
{
	...
	for(std::list<std::string>::const_iterator it = inputList.begin(); it != inputList.end(); ++it)
	{
		...
		StringVector::iterator lastWordMatch = wordList.begin();

		int32_t matchCount = 0;
		StringVector keywordList = explodeString(keywords, ";");
		for(StringVector::iterator kit = keywordList.begin(); kit != keywordList.end(); ++kit)
		{
			...
			{
				StringVector::iterator wit = wordList.end();
...
LOGGER_DEFINITION(NpcScriptInterface);
...
bool NpcScriptInterface::loadNpcLib(std::string file)
{
	...
	if(!loadFile(file))
	{
		LOGe("[NpcScriptInterface::loadNpcLib] Cannot load " << file);
		return false;
	}
	...
	NpcEventsHandler(npc)
{
	...
	{
		LOGe("[NpcScript::NpcScript] Cannot load script " << file << ": " << m_interface->getLastError());
...
void NpcScript::onCreatureAppear(const Creature* creature)
{
	...
	if(m_interface->reserveEnv())
	{
		...
		LOGe("[NpcScript::onCreatureAppear] NPC Name: " << m_npc->getName() << " - Call stack overflow");
}
...
void NpcScript::onCreatureDisappear(const Creature* creature)
{
	...
	if(m_interface->reserveEnv())
	{
		...
		LOGe("[NpcScript::onCreatureDisappear] NPC Name: " << m_npc->getName() << " - Call stack overflow");
}
...
void NpcScript::onCreatureMove(const Creature* creature, const Position& oldPos, const Position& newPos)
{
	...
	if(m_interface->reserveEnv())
	{
		...
		LOGe("[NpcScript::onCreatureMove] NPC Name: " << m_npc->getName() << " - Call stack overflow");
}
...
void NpcScript::onCreatureSay(const Creature* creature, SpeakClasses type, const std::string& text, Position* pos/* = nullptr*/)
{
	...
	if(m_interface->reserveEnv())
	{
		...
		LOGe("[NpcScript::onCreatureSay] NPC Name: " << m_npc->getName() << " - Call stack overflow");
}
	...
	uint8_t count, uint8_t amount, bool ignoreCap, bool inBackpacks)
{
	...
	if(m_interface->reserveEnv())
	{
		...
		LOGe("[NpcScript::onPlayerTrade] NPC Name: " << m_npc->getName() << " - Call stack overflow");
}
...
void NpcScript::onPlayerCloseChannel(const Player* player)
{
	...
	if(m_interface->reserveEnv())
	{
		...
		LOGe("[NpcScript::onPlayerCloseChannel] NPC Name: " << m_npc->getName() << " - Call stack overflow");
}
...
void NpcScript::onPlayerEndTrade(const Player* player)
{
	...
	if(m_interface->reserveEnv())
	{
		...
		LOGe("[NpcScript::onPlayerEndTrade] NPC Name: " << m_npc->getName() << " - Call stack overflow");
}
...
void NpcScript::onThink()
{
	...
	if(m_interface->reserveEnv())
	{
		...
		LOGe("[NpcScript::onThink] NPC Name: " << m_npc->getName() << " - Call stack overflow");
}
