...
void Party::shareExperience(double experience, bool fromMonster, bool multiplied)
{
	...
	if(experience > (double)server.configManager().getNumber(ConfigManager::EXTRA_PARTY_LIMIT))
		shareExperience += (experience * (double)(server.configManager().getNumber(ConfigManager::EXTRA_PARTY_PERCENT) / 100));
...
bool Party::canUseSharedExperience(const Player* player, uint32_t highestLevel/* = 0*/) const
{
	...
	if(player->getLevel() < (uint32_t)std::ceil((double)highestLevel * server.configManager().getDouble(
		ConfigManager::PARTY_DIFFERENCE)) || !Position::areInRange(Position(
		server.configManager().getNumber(ConfigManager::PARTY_RADIUS_X), server.configManager().getNumber(
		ConfigManager::PARTY_RADIUS_Y), server.configManager().getNumber(ConfigManager::PARTY_RADIUS_Z)),
