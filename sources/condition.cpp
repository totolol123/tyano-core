...
bool ConditionDamage::doDamage(Creature* creature, int32_t damage)
{
	...
	return server.game().combatChangeHealth(combatType, attacker, creature, damage);
}
...
bool ConditionSpeed::startCondition(Creature* creature)
{
	...
	server.game().changeSpeed(creature, speedDelta);
...
void ConditionSpeed::addCondition(Creature* creature, const Condition* addCondition)
{
	...
	if(newSpeedChange)
		server.game().changeSpeed(creature, newSpeedChange);
}
...
{
	if(!outfits.empty())
		server.game().internalCreatureChangeOutfit(creature, creature->getDefaultOutfit(), true);
}
...
bool ConditionLight::startCondition(Creature* creature)
{
	...
	server.game().changeLight(creature);
	return Condition::startCondition(creature);
}
