...
LOGGER_DEFINITION(Monsters);


uint16_t Monsters::getLootRandom()
{
	return (uint16_t)std::ceil((double)random_range(0, MAX_LOOTCHANCE) / server.configManager().getDouble(ConfigManager::RATE_LOOT));
}
...
void MonsterType::dropLoot(Container* corpse)
{
	...
	if(message == LOOTMSG_IGNORE)
		message = (LootMessage_t)server.configManager().getNumber(ConfigManager::LOOT_MESSAGE);
	...
	if(owner->getParty() && message > LOOTMSG_PLAYER)
		owner->getParty()->broadcastMessage((MessageClasses)server.configManager().getNumber(ConfigManager::LOOT_MESSAGE_TYPE), ss.str());
	else if(message == LOOTMSG_PLAYER || message == LOOTMSG_BOTH)
		owner->sendTextMessage((MessageClasses)server.configManager().getNumber(ConfigManager::LOOT_MESSAGE_TYPE), ss.str());
}

boost::intrusive_ptr<Item> MonsterType::createLoot(const LootBlock& lootBlock)
{
	uint16_t kindId = lootBlock.ids[0], random = Monsters::getLootRandom();
	if(lootBlock.ids.size() > 1)
		kindId = lootBlock.ids[random_range((size_t)0, lootBlock.ids.size() - 1)];
	...
	if(kind->stackable)
	{
		if(random < lootBlock.chance)
			tmpItem = Item::CreateItem(kindId, (random % lootBlock.count + 1));
	}
	else if(random < lootBlock.chance)
		tmpItem = Item::CreateItem(kindId, 0);
...
bool MonsterType::createChildLoot(Container* parent, const LootBlock& lootBlock)
{
	...
	for(; it != lootBlock.childLoot.end() && !parent->full(); ++it)
	{
		...
		{
			if(boost::intrusive_ptr<Container> container = tmpItem->getContainer())
			{
				if(createChildLoot(container.get(), (*it)))
					parent->__internalAddThing(container.get());
			}
			else
				parent->__internalAddThing(tmpItem.get());
		}
	}
...
{
	loaded = false;
	xmlDocPtr doc = xmlParseFile(getFilePath(FileType::OTHER, "monster/monsters.xml").c_str());
	if(!doc)
	{
		LOGe("[Monsters::loadFromXml] Cannot load monsters file: " << getLastXMLError());
		return false;
	}
	...
	{
		LOGe("[Monsters::loadFromXml] Malformed monsters file.");
		xmlFreeDoc(doc);
		return false;
	}
	...
	while(p)
	{
		...
		{
			LOGe("[Monsters::loadFromXml] Unknown node name (" << p->name << ").");
			p = p->next;
			continue;
		}
		...
		{
			file = getFilePath(FileType::OTHER, "monster/" + file);
...
bool Monsters::deserializeSpell(xmlNodePtr node, spellBlock_t& sb, const std::string& description)
{
	...
	if((sb.spell = server.spells().getSpellByName(name)))
		return true;

	CombatSpellP combatSpell = nullptr;
	...
	if(isScripted)
	{
		...
		combatSpell = std::make_shared<CombatSpell>(nullptr, needTarget, needDirection);
		if(!combatSpell->loadScript(getFilePath(FileType::OTHER, server.spells().getScriptBaseName() + "/scripts/" + scriptName), true))
		{
			return false;
		}
	...
	else
	{
		...
		else if(tmpName == "speed")
		{
			...
			for(xmlNodePtr tmpNode = node->children; tmpNode; tmpNode = tmpNode->next)
			{
				...
				{
					if(MonsterType* mType = server.monsters().getMonsterType(strValue))
		...
		{
			delete combat;
			LOGe("[Monsters::deserializeSpell] " << description << " - Unknown spell name: " << name);
			return false;
		}
					...
					if(tmpStrValue == "shooteffect")
					{
						if(readXMLString(attributeNode, "value", strValue))
						{
							...
							if(shoot != SHOOT_EFFECT_UNKNOWN)
								...
								LOGw("[Monsters::deserializeSpell] " << description << " - Unknown shootEffect: " << strValue);
						}
					}
					else if(tmpStrValue == "areaeffect")
					{
						if(readXMLString(attributeNode, "value", strValue))
						{
							...
							if(effect != MAGIC_EFFECT_UNKNOWN)
								...
								LOGw("[Monsters::deserializeSpell] " << description << " - Unknown areaEffect: " << strValue);
						}
					}
					else
						LOGw("[Monsters::deserializeSpells] Effect type \"" << strValue << "\" does not exist.");
				}
			}
...
#define SHOW_XML_WARNING(desc)  LOGw("[Monsters::loadMonster] " << desc << ". (" << file << ")")
#define SHOW_XML_ERROR(desc)    LOGe("[Monsters::loadMonster] " << desc << ". (" << file << ")")
...
bool Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading/* = false*/)
{
	...
	{
		LOGw("[Monsters::loadMonster] Duplicate registered monster with name: " << monsterName);
		return true;
	}
	...
	if(!doc)
	{
		LOGw("[Monsters::loadMonster] Cannot load monster (" << monsterName << ") file (" << file << "): " << getLastXMLError());
		return false;
	}
	...
	{
		LOGe("[Monsters::loadMonster] Malformed monster (" << monsterName << ") file (" << file << ").");
		xmlFreeDoc(doc);
		return false;
	}
...
bool Monsters::loadLoot(xmlNodePtr node, LootBlock& lootBlock)
{
	...
	if(readXMLString(node, "id", strValue) || readXMLString(node, "ids", strValue))
	{
		...
		for(IntegerVec::iterator it = idsVec.begin(); it != idsVec.end(); ++it)
		{
			...
			ItemKindPC kind = items[*it];
			if(kind && kind->isContainer())
				loadChildLoot(node, lootBlock);
		}
	}
	else if(readXMLString(node, "name", strValue) || readXMLString(node, "names", strValue))
	{
		StringVector names = explodeString(strValue, ";");
		for(StringVector::iterator it = names.begin(); it != names.end(); ++it)
		{
			ItemKindPC kind = items.getKindByName(strValue);
			if(!kind)
				continue;

			lootBlock.ids.push_back(kind->id);
			if(kind->isContainer())
				loadChildLoot(node, lootBlock);
		}
	}
