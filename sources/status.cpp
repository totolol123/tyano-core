...
{
	LOGt("ProtocolStatus::deleteProtocolTask()");
...
std::string Status::getStatusString(bool sendPlayers) const
{
	...
	xmlSetProp(p, (const xmlChar*)"ip", (const xmlChar*)server.configManager().getString(ConfigManager::IP).c_str());
	xmlSetProp(p, (const xmlChar*)"servername", (const xmlChar*)server.configManager().getString(ConfigManager::SERVER_NAME).c_str());
	sprintf(buffer, "%d", server.configManager().getNumber(ConfigManager::LOGIN_PORT));
	xmlSetProp(p, (const xmlChar*)"port", (const xmlChar*)buffer);
	xmlSetProp(p, (const xmlChar*)"location", (const xmlChar*)server.configManager().getString(ConfigManager::LOCATION).c_str());
	xmlSetProp(p, (const xmlChar*)"url", (const xmlChar*)server.configManager().getString(ConfigManager::URL).c_str());
	...
	xmlSetProp(p, (const xmlChar*)"name", (const xmlChar*)server.configManager().getString(ConfigManager::OWNER_NAME).c_str());
	xmlSetProp(p, (const xmlChar*)"email", (const xmlChar*)server.configManager().getString(ConfigManager::OWNER_EMAIL).c_str());
	xmlAddChild(root, p);
	...
	sprintf(buffer, "%d", server.configManager().getNumber(ConfigManager::MAX_PLAYERS));
	xmlSetProp(p, (const xmlChar*)"max", (const xmlChar*)buffer);
	sprintf(buffer, "%d", server.game().getPlayersRecord());
	...
	sprintf(buffer, "%d", server.game().getMonstersOnline());
	xmlSetProp(p, (const xmlChar*)"total", (const xmlChar*)buffer);
	xmlAddChild(root, p);

	p = xmlNewNode(nullptr,(const xmlChar*)"npcs");
	sprintf(buffer, "%d", server.game().getNpcsOnline());
	xmlSetProp(p, (const xmlChar*)"total", (const xmlChar*)buffer);
	xmlAddChild(root, p);

	p = xmlNewNode(nullptr,(const xmlChar*)"map");
	xmlSetProp(p, (const xmlChar*)"name", (const xmlChar*)m_mapName.c_str());
	xmlSetProp(p, (const xmlChar*)"author", (const xmlChar*)server.configManager().getString(ConfigManager::MAP_AUTHOR).c_str());

	uint32_t mapWidth, mapHeight;
	server.game().getMapDimensions(mapWidth, mapHeight);
	...
	xmlNewTextChild(root, nullptr, (const xmlChar*)"motd", (const xmlChar*)server.configManager().getString(ConfigManager::MOTD).c_str());
...
void Status::getInfo(uint32_t requestedInfo, OutputMessage_ptr output, NetworkMessage& msg) const
{
	...
	{
		output->AddByte(0x10);
		output->AddString(server.configManager().getString(ConfigManager::SERVER_NAME).c_str());
		output->AddString(server.configManager().getString(ConfigManager::IP).c_str());
	...
	{
		output->AddByte(0x11);
		output->AddString(server.configManager().getString(ConfigManager::OWNER_NAME).c_str());
		output->AddString(server.configManager().getString(ConfigManager::OWNER_EMAIL).c_str());
	}

	if(requestedInfo & REQUEST_MISC_SERVER_INFO)
	{
		output->AddByte(0x12);
		output->AddString(server.configManager().getString(ConfigManager::MOTD).c_str());
		output->AddString(server.configManager().getString(ConfigManager::LOCATION).c_str());
		output->AddString(server.configManager().getString(ConfigManager::URL).c_str());
	...
	{
		output->AddByte(0x20);
		output->AddU32(server.game().getPlayersOnline());
		output->AddU32(server.configManager().getNumber(ConfigManager::MAX_PLAYERS));
		output->AddU32(server.game().getPlayersRecord());
	}
	...
	if(requestedInfo & REQUEST_SERVER_MAP_INFO)
	{
		...
		output->AddString(server.configManager().getString(ConfigManager::MAP_AUTHOR).c_str());
	...
	if(requestedInfo & REQUEST_PLAYER_STATUS_INFO)
	{
		...
		Player* p = nullptr;
		if(server.game().getPlayerByNameWildcard(name, p) == RET_NOERROR && !p->isGhost())
			output->AddByte(0x01);
		else
			output->AddByte(0x00);
	}
