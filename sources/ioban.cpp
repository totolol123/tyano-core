	...
	std::string comment, uint32_t gamemaster, int16_t channelId/* = -1*/, std::string statement/* = ""*/) const
{
	...
	query << ", '-1', " << time(nullptr) << ", " << gamemaster << ", " << db.escapeString(comment.c_str());
...
bool IOBan::clearTemporials() const
{
	...
	query << "UPDATE `bans` SET `active` = 0 WHERE `expires` <= " << time(nullptr) << " AND `expires` >= 0 AND `active` = 1" << db.getUpdateLimiter();
	return db.executeQuery(query.str());
}
