		...
		m_vocationMap(vocationMap)
{
	...
	{
		m_file.reset(new std::ofstream(getFilePath(FileType::LOG, (std::string)"chat/" + server.configManager().getString(
...
{
	xmlDocPtr doc = xmlParseFile(getFilePath(FileType::XML, "channels.xml").c_str());
	if(!doc)
	{
		LOGe("[Chat::loadFromXml] Cannot load channels file." << getLastXMLError());
		return false;
	}
	...
	{
		LOGe("[Chat::loadFromXml] Malformed channels file");
		xmlFreeDoc(doc);
		return false;
	}
...
bool Chat::parseChannelNode(xmlNodePtr p)
{
	...
	{
		LOGe("[Chat::loadFromXml] Invalid or not specified channel id.");
		return false;
	}
	...
	{
		LOGe("[Chat::loadFromXml] Duplicated channel with id: " << id << ".");
		return false;
	}

	if(!readXMLString(p, "name", strValue))
	{
		LOGe("[Chat::loadFromXml] Missing name for channel with id: " << id << ".");
		return false;
	}
	...
	if(readXMLInteger(p, "muted", intValue))
	{
		...
		if(readXMLInteger(p, "conditionId", intValue))
		{
			...
			if(conditionId < 2)
				LOGw("[Chat::parseChannelNode] Using reserved muted condition sub id (" << conditionId << ")");
		}
...
bool Chat::talkToChannel(Player* player, SpeakClasses type, const std::string& text, uint16_t channelId)
{
	...
	if(channelId != CHANNEL_GUILD || !server.configManager().getBool(ConfigManager::INGAME_GUILD_MANAGEMENT)
		|| (text[0] != '!' && text[0] != '/'))
	{
	...
	else if(text.substr(1, 6) == "invite")
	{
		if(player->getGuildLevel() > GUILDLEVEL_MEMBER)
		{
			if(text.length() > 7)
			{
				...
				if(server.game().getPlayerByNameWildcard(param, paramPlayer) == RET_NOERROR)
				{
	...
	{
		StringVector params = explodeString(text.substr(6), ",");
		if(params.size() >= 2)
		{
			...
			trimString(param2);
			Player* paramPlayer = nullptr;
			if(server.game().getPlayerByNameWildcard(param1, paramPlayer) == RET_NOERROR)
			{
				if(paramPlayer->getGuildId())
				{
					if(param2.length() > 2)
	...
	{
		StringVector params = explodeString(text.substr(13), ",");
		if(params.size() >= 2)
		{
			std::string param1 = params[0], param2 = params[1];
			trimString(param1);
