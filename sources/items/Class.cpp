...
LOGGER_DEFINITION(ts::items::Class<Kind>);



Class<Kind>::Class(const std::string& id, const std::string& name, const attributes::SchemeP& attributesScheme)
...
attributes::SchemeP Class<Kind>::getAttributesScheme() const {
	return _attributesScheme;
}


const std::string& Class<Kind>::getName() const {
	return _name;
}


Class<Kind>::KindP Class<Kind>::createKindFromXmlNode(xmlNodePtr node) const {
	if (node == nullptr) {
		assert(node != nullptr);
		return nullptr;
	}

#warning remove  if (kindId > 20000 && kindId < 20100) kindId -= 20000;
...
#define warning(x)  LOGw(x << " in " << node->doc->name << ":" << node->line)
	...
	if (!xml::attribute(*node, "id", id)) {
		error("Invalid item id. Value must be a number in range 1.." << std::numeric_limits<uint16_t>::max());
		return nullptr;
	}

	KindP kind = createKind();
	kind->setId(id);
...
void Class<Kind>::processKindParameter(KindT& kind, const std::string& name, const std::string& value, xmlNodePtr deprecatedNode) const {
