...
LOGGER_DEFINITION(ts::items::Kind);


Kind::Kind(const ClassP& clazz)
	: _armor(0),
	  _class(clazz),
	  _defense(0),
	  _defenseBonus(0),
	  ...
	  _clientId(0),
	  _decayedItemId(0),
	  _equipmentCounterpartItemId(0),
	  _groundSpeed(0),
	  _id(0),
	  _maximumTextLength(0),
	  _rotatedItemId(0),
	  _writtenCounterpartItemId(0),
...
#warning add to worth map & check duplicates
	...
	if (name == "armor") {
	...
	else if (name == "decayedItemId" || name == "decayto") {
		_setParameter(name, value, _decayedItemId, "decayedItemId");
	}
	...
	else if (name == "equipmentCounterpartItemId" || name == "transformequipto" || name == "transformdeequipto") {
		_setParameter(name, value, _equipmentCounterpartItemId, "equipmentCounterpartItemId");
	}
		...
		_setParameter(name, value, _healthRegenerationInterval, "healthRegenerationInterval");
	}
	else if (name == "lifetime" || name == "duration") {
		_setParameter(name, value, _lifetime, "lifetime");
	}
		...
		_setParameter(name, value, _manaRegenerationInterval, "manaRegenerationInterval");
	}
	...
	else if (name == "rotatedItemId" || name == "rotateto") {
		_setParameter(name, value, _rotatedItemId, "rotatedItemId");
	}
	...
	else if (name == "writtenCounterpartItemId" || name == "writeonceitemid") {
		_setParameter(name, value, _writtenCounterpartItemId, "writtenCounterpartItemId");
	}
	else {
		return false;
	}
...
void Kind::_setAttributes(const std::string& name, xmlNode& node) {
	for (xmlNodePtr child = node.children; child != nullptr; child = child->next) {
		...
		Attribute attribute = creatures::getAttributeWithId(id);
		switch (attribute) {
		case Attribute::NONE:
