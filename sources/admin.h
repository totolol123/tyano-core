...
class Admin
{
	public:
		...
		static boost::intrusive_ptr<Item> createMail(const std::string xmlData, std::string& name, uint32_t& depotId);
...
class ProtocolAdmin : public Protocol
{
	public:
		...
		ProtocolAdmin(const Connection_ptr& connection): Protocol(connection)
		{
			m_beganScript = false;
	...
	protected:
		...
		void adminCommandExecuteScript(const std::string& script);
	...
	private:
		void addLogLine(LogType type, std::string message);

		int32_t m_loginTries;
		ProtocolState_t m_state;
		uint32_t m_lastCommand, m_startTime;
		std::stringstream m_scriptBuffer;
		bool m_beganScript;
};
