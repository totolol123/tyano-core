...
bool Vocations::parseVocationNode(xmlNodePtr p)
{
	...
	{
		LOGe("[Vocations::parseVocationNode] Missing vocation id.");
		return false;
	}
	...
	for(xmlNodePtr configNode = p->children; configNode; configNode = configNode->next)
	{
		if(!xmlStrcmp(configNode->name, (const xmlChar*)"skill"))
		{
			...
			if(readXMLInteger(configNode, "id", intValue))
			{
				...
				{
					LOGe("[Vocations::parseVocationNode] No valid skill id (" << intValue << ").");
					continue;
				}
...
{
	xmlDocPtr doc = xmlParseFile(getFilePath(FileType::XML,"vocations.xml").c_str());
	if(!doc)
	{
		LOGe("[Vocations::loadFromXml] Cannot load vocations file: " << getLastXMLError());
		return false;
	}
	...
	{
		LOGe("[Vocations::loadFromXml] Malformed vocations file.");
		xmlFreeDoc(doc);
		return false;
	}
...
Vocation* Vocations::getVocation(uint32_t vocId)
{
	...
	LOGe("[Vocations::getVocation] Vocation " << vocId << " not found.");
