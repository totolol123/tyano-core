...
void ScriptEnviroment::clearAll() {
	for(AreaMap::iterator it = m_areaMap.begin(); it != m_areaMap.end(); ++it)
		delete it->second;
	m_areaMap.clear();
...
{
	if(!server.configManager().getBool(ConfigManager::SAVE_GLOBAL_STORAGE))
		return true;

	Database& db = server.database();
	DBQuery query;

	query << "DELETE FROM `global_storage` WHERE `world_id` = " << server.configManager().getNumber(ConfigManager::WORLD_ID) << ";";
	...
	for(StorageMap::const_iterator it = m_storageMap.begin(); it != m_storageMap.end(); ++it)
	{
		...
		sprintf(buffer, "%u, %u, %s", it->first, server.configManager().getNumber(ConfigManager::WORLD_ID), db.escapeString(it->second).c_str());
...
void ScriptEnviroment::addUniqueThing(Thing* thing)
{
	...
	if(m_globalMap[item->getUniqueId()])
	{
		if(item->getActionId() != 2000) //scripted quest system
			LOGw("Duplicate uniqueId " << item->getUniqueId());
	}
	else
...
void ScriptEnviroment::insertThing(uint32_t uid, Thing* thing)
{
	if(!m_localMap[uid])
		...
		LOGe("[ScriptEnviroment::insertThing] Thing uid already taken");
}
...
LOGGER_DEFINITION(LuaScriptInterface);
...
bool LuaScriptInterface::call(const std::string& text, std::string& result) {
	bool success = false;

	if (reserveEnv()) {
		if (luaL_loadbuffer(m_luaState, text.c_str(), text.length(), "call") == 0) {
			m_loadingFile = "call";

			ScriptEnviroment* env = getEnv();
			env->setScriptId(EVENT_ID_USER, this);
		...
		result = popString(m_luaState);
		if (!success) {
			error(nullptr, result);
		}

		releaseEnv();
	}
	else {
		result = "Call stack overflow";
	}
...
bool LuaScriptInterface::loadFile(const std::string& file, Npc* npc/* = nullptr*/)
{
	...
	{
		m_lastError = popString(m_luaState);
		LOGe("[LuaScriptInterface::loadFile] " << m_lastError);
		return false;
	}
...
void LuaScriptInterface::error(const char* function, const std::string& desc)
{
	...
	if(interface)
	{
		LOGe(interface->getName() << ": ");
		if(callback)
			LOGe("\tIn a callback: " << interface->getScript(callback));

		if(timer)
			LOGe("\t" << (callback ? "from" : "In") << " a timer event called from:");

		LOGe("\t" << interface->getScript(script));
		LOGe("\tDescription: " << event);
	}
	else
		LOGe("LUA: " << event);
...
bool LuaScriptInterface::initState()
{
	...
	registerFunctions();
	if(!loadDirectory(getFilePath(FileType::OTHER, "lib/"), nullptr))
		LOGe("[LuaScriptInterface::initState] Cannot load " << getFilePath(FileType::OTHER, "lib/"));
...
void LuaScriptInterface::executeTimer(uint32_t eventIndex)
{
	...
	if(it != m_timerEvents.end())
	{
		...
		if(reserveEnv())
		{
			...
			LOGe("Call stack overflow. LuaScriptInterface::executeTimer");
...
void LuaScriptInterface::dumpStack(lua_State* L/* = nullptr*/)
{
	...
	LOGe("Stack size: " << stack);
	for(int32_t i = 1; i <= stack ; ++i)
		LOGe(lua_typename(m_luaState, lua_type(m_luaState, -i)) << " " << lua_topointer(m_luaState, -i));
}
...
void LuaScriptInterface::getValue(const std::string& key, lua_State* L, lua_State* luaState)
{
	lua_getglobal(L, key.c_str());
	moveValue(L, luaState);
}
...
int32_t LuaScriptInterface::internalGetPlayerInfo(lua_State* L, PlayerInfo_t info)
{
	...
	switch(info)
	{
		...
		case PlayerInfoBalance:
			value = (server.configManager().getBool(ConfigManager::BANK_SYSTEM) ? player->balance : 0);
			break;
...
int32_t LuaScriptInterface::luaDoPlayerLearnInstantSpell(lua_State* L)
{
	...
	InstantSpellP spell = server.spells().getInstantSpellByName(spellName);
	if(!spell)
	{
		lua_pushboolean(L, false);
		return 1;
...
int32_t LuaScriptInterface::luaGetPlayerInstantSpellCount(lua_State* L)
{
	...
	ScriptEnviroment* env = getEnv();
	if(Player* player = env->getPlayerByUID(popNumber(L)))
		lua_pushnumber(L, server.spells().getInstantSpellCount(player));
	else
	{
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
		lua_pushboolean(L, false);
...
int32_t LuaScriptInterface::luaGetPlayerInstantSpellInfo(lua_State* L)
{
	...
	InstantSpellP spell = server.spells().getInstantSpellByIndex(player, index);
	if(!spell)
	{
...
int32_t LuaScriptInterface::luaGetInstantSpellInfo(lua_State* L)
{
	...
	InstantSpellP spell = server.spells().getInstantSpellByName(popString(L));
	if(!spell)
	{
		errorEx(getError(LUA_ERROR_SPELL_NOT_FOUND));
		lua_pushboolean(L, false);
...
int32_t LuaScriptInterface::luaDoRemoveItem(lua_State* L)
{
	...
	if(server.game().internalRemoveItem(nullptr, item, count) != RET_NOERROR)
	{
		lua_pushboolean(L, false);
		return 1;
	}
...
int32_t LuaScriptInterface::luaGetSearchString(lua_State* L)
{
	...
	if(!toPos.x || !toPos.y || !fromPos.x || !fromPos.y)
	{
		...
		lua_pushstring(L, server.game().getSearchString(fromPos, toPos, fromIsCreature, toIsCreature).c_str());
...
int32_t LuaScriptInterface::luaDoTeleportThing(lua_State* L)
{
	...
	if(Thing* tmp = env->getThingByUID(popNumber(L)))
		lua_pushboolean(L, server.game().internalTeleport(tmp, pos, pushMove) == RET_NOERROR);
	else
	{
...
int32_t LuaScriptInterface::luaDoTransformItem(lua_State* L)
{
	...
	ItemKindPC kind = server.items()[newId];
...
int32_t LuaScriptInterface::luaDoCreatureSay(lua_State* L)
{
	...
	if(params > 5)
		lua_pushboolean(L, server.game().internalCreatureSay(creature, type, text, ghost, &list, &pos));
	else
		lua_pushboolean(L, server.game().internalCreatureSay(creature, type, text, ghost, &list));
...
int32_t LuaScriptInterface::luaDoSendMagicEffect(lua_State* L)
{
	...
	if(!list.empty())
		server.game().addMagicEffect(list, pos, type);
	else
		server.game().addMagicEffect(pos, type);
...
int32_t LuaScriptInterface::luaDoSendDistanceShoot(lua_State* L)
{
	...
	if(!list.empty())
		server.game().addDistanceEffect(list, fromPos, toPos, type);
	else
		server.game().addDistanceEffect(fromPos, toPos, type);
...
int32_t LuaScriptInterface::luaDoCreatureSetHideHealth(lua_State* L)
{
	...
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		...
		server.game().addCreatureHealth(creature);
		lua_pushboolean(L, true);
	}
	else
	{
...
int32_t LuaScriptInterface::luaDoCreatureAddHealth(lua_State* L)
{
	...
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		if(healthChange) //do not post with 0 value
			server.game().combatChangeHealth(healthChange < 1 ? COMBAT_UNDEFINEDDAMAGE : COMBAT_HEALING,
...
int32_t LuaScriptInterface::luaDoCreatureAddMana(lua_State* L)
{
	...
	{
		if(aggressive)
			server.game().combatChangeMana(nullptr, creature, manaChange);
		else
...
int32_t LuaScriptInterface::luaDoPlayerAddItem(lua_State* L)
{
	...
	while(itemCount > 0)
	{
		...
		boost::intrusive_ptr<Item> newItem = Item::CreateItem(itemId, stackCount);
		if(!newItem)
		{
			errorEx(getError(LUA_ERROR_ITEM_NOT_FOUND));
			lua_pushboolean(L, false);
		...
		ReturnValue ret = server.game().internalPlayerAddItem(nullptr, player, newItem.get(), canDropOnMap);
		if(ret != RET_NOERROR)
		{
			lua_pushboolean(L, false);
			return 1;
		...
		if(newItem->getParent())
			lua_pushnumber(L, env->addThing(newItem.get()));
...
int32_t LuaScriptInterface::luaDoPlayerAddItemEx(lua_State* L)
{
	...
	if(item->getParent() == &VirtualCylinder::virtualCylinder)
		lua_pushnumber(L, server.game().internalPlayerAddItem(nullptr, player, item, canDropOnMap));
	else
		lua_pushboolean(L, false);
...
int32_t LuaScriptInterface::luaDoTileAddItemEx(lua_State* L)
{
	...
	if(item->getParent() == &VirtualCylinder::virtualCylinder)
		lua_pushnumber(L, server.game().internalAddItem(nullptr, tile, item));
	else
		lua_pushboolean(L, false);
...
int32_t LuaScriptInterface::luaDoRelocate(lua_State* L)
{
	...
	Tile* toTile = server.game().getTile(toPos.x, toPos.y, toPos.z);
	if(!toTile)
	{
		errorEx(getError(LUA_ERROR_TILE_NOT_FOUND));
		lua_pushboolean(L, false);
		...
		for(int32_t i = fromTile->getThingCount() - 1; i >= 0; --i)
		{
			...
			if(thing)
			{
				if(Item* item = thing->getItem())
				{
					...
					if(!kind->isGroundTile() && !kind->alwaysOnTop && !kind->isMagicField())
						server.game().internalTeleport(item, toPos, false, FLAG_IGNORENOTMOVEABLE);
				}
				else if(creatures)
				{
					Creature* creature = thing->getCreature();
					if(creature)
						server.game().internalTeleport(creature, toPos, true);
				}
			}
		}
	}
...
int32_t LuaScriptInterface::luaDoSendAnimatedText(lua_State* L)
{
	...
	if(!list.empty())
		server.game().addAnimatedText(list, pos, color, text);
	else
		server.game().addAnimatedText(pos, color, text);
...
int32_t LuaScriptInterface::luaGetTileItemById(lua_State* L)
{
	...
	Item* item = server.game().findItemOfType(tile, itemId, false, subType);
	if(!item)
	{
		pushThing(L, nullptr, 0);
		return 1;
...
int32_t LuaScriptInterface::luaDoCreateItemEx(lua_State* L)
{
	...
	ItemKindPC kind = server.items()[(uint32_t)popNumber(L)];
	if (!kind) {
		errorEx(getError(LUA_ERROR_ITEM_NOT_FOUND));
		lua_pushboolean(L, false);
		return 1;
	...
	if(kind->stackable && count > 100)
		count = 100;

	boost::intrusive_ptr<Item> newItem = Item::CreateItem(kind->id, count);
	if(!newItem)
	{
		errorEx(getError(LUA_ERROR_ITEM_NOT_FOUND));
		lua_pushboolean(L, false);
	...
	env->addTempItem(env, newItem.get());
...
int32_t LuaScriptInterface::luaDoCreateTeleport(lua_State* L)
{
	...
	Tile* tile = server.game().getMap()->getTile(createPos);
	if(!tile)
	{
		errorEx(getError(LUA_ERROR_TILE_NOT_FOUND));
		lua_pushboolean(L, false);
	...
	if(server.game().internalAddItem(nullptr, tile, newTeleport, INDEX_WHEREEVER, FLAG_NOLIMIT) != RET_NOERROR)
	{
		lua_pushboolean(L, false);
		return 1;
	}
...
int32_t LuaScriptInterface::luaGetTileInfo(lua_State* L)
{
	...
	{
		ScriptEnviroment* env = getEnv();
		pushThing(L, tile->ground.get(), env->addThing(tile->ground.get()));
...
int32_t LuaScriptInterface::luaDoCreateMonster(lua_State* L)
{
	...
	ScriptEnviroment* env = getEnv();
	lua_pushnumber(L, env->addThing(monster.get()));
	return 1;
}
...
int32_t LuaScriptInterface::luaDoPlayerAddMoney(lua_State* L)
{
	...
	if(Player* player = env->getPlayerByUID(popNumber(L)))
	{
		server.game().addMoney(player, money);
		lua_pushboolean(L, true);
	}
	else
	{
...
int32_t LuaScriptInterface::luaDoPlayerRemoveMoney(lua_State* L)
{
	...
	ScriptEnviroment* env = getEnv();
	if(Player* player = env->getPlayerByUID(popNumber(L)))
		lua_pushboolean(L, server.game().removeMoney(player, money));
	else
	{
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
		lua_pushboolean(L, false);
...
int32_t LuaScriptInterface::luaGetPlayerMoney(lua_State* L)
{
	...
	ScriptEnviroment* env = getEnv();
	if(Player* player = env->getPlayerByUID(popNumber(L)))
		lua_pushnumber(L, server.game().getMoney(player));
	else
	{
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
		lua_pushboolean(L, false);
...
{
	lua_pushnumber(L, (uint32_t)server.game().getWorldType());
	return 1;
}
...
{
	//getWorldTime()
	lua_pushnumber(L, server.game().getLightHour());
	return 1;
}
...
int32_t LuaScriptInterface::luaGetWorldCreatures(lua_State* L)
{
	...
	{
		case 0:
			value = server.game().getPlayersOnline();
			break;
		case 1:
			value = server.game().getMonstersOnline();
			break;
		case 2:
			value = server.game().getNpcsOnline();
			break;
		case 3:
			value = server.game().getCreaturesOnline();
			break;
		default:
			lua_pushboolean(L, false);
			return 1;
...
int32_t LuaScriptInterface::luaGetPlayerItemById(lua_State* L)
{
	...
	Item* item = server.game().findItemOfType(player, itemId, deepSearch, subType);
	if(!item)
	{
		pushThing(L, nullptr, 0);
		return 1;
...
int32_t LuaScriptInterface::luaSetCombatFormula(lua_State* L)
{
	...
	double minm = server.configManager().getDouble(ConfigManager::FORMULA_MAGIC), maxm = minm,
		minl = server.configManager().getDouble(ConfigManager::FORMULA_LEVEL), maxl = minl;
	if(params > 8)
	{
	...
	switch(var.type)
	{
		...
		case VARIANT_TARGETPOSITION:
		{
			if(!combat->hasArea())
			{
				...
				server.game().addMagicEffect(var.pos, MAGIC_EFFECT_POFF);
			}
			else
				combat->doCombat(creature, var.pos);
...
int32_t LuaScriptInterface::luaDoSummonMonster(lua_State* L)
{
	...
	lua_pushnumber(L, server.game().placeSummon(creature, name));
	return 1;
}
...
{
	//getMonsterInfo(name)
	const MonsterType* mType = server.monsters().getMonsterType(popString(L));
	if(!mType)
	{
		errorEx(getError(LUA_ERROR_MONSTER_NOT_FOUND));
		lua_pushboolean(L, false);
...
{
	//getTalkactionList()
	TalkActionsMap::const_iterator it = server.talkActions().getFirstTalk();
	lua_newtable(L);
	for(uint32_t i = 1; it != server.talkActions().getLastTalk(); ++it, ++i)
	{
		createTable(L, i);
...
int32_t LuaScriptInterface::luaGetExperienceStageList(lua_State* L)
{
	...
	StageList::const_iterator it = server.game().getFirstStage();
	lua_newtable(L);
	for(uint32_t i = 1; it != server.game().getLastStage(); ++it, ++i)
	{
		createTable(L, i);
...
int32_t LuaScriptInterface::luaDoChangeSpeed(lua_State* L)
{
	...
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		server.game().changeSpeed(creature, delta);
		lua_pushboolean(L, true);
	}
	else
	{
...
int32_t LuaScriptInterface::luaDoMoveCreature(lua_State* L)
{
	...
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
		lua_pushnumber(L, server.game().internalMoveCreature(creature, (Direction)direction, FLAG_NOLIMIT));
	else
	{
		errorEx(getError(LUA_ERROR_CREATURE_NOT_FOUND));
		lua_pushboolean(L, false);
...
int32_t LuaScriptInterface::luaGetCreatureByName(lua_State* L)
{
	//getCreatureByName(name)
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = server.game().getCreatureByName(popString(L)))
...
int32_t LuaScriptInterface::luaGetPlayerByGUIDEx(lua_State* L)
{
	//getPlayerByGUIDEx(guid)
	ScriptEnviroment* env = getEnv();
...
int32_t LuaScriptInterface::luaGetPlayerByNameWildcard(lua_State* L)
{
	...
	ScriptEnviroment* env = getEnv();
	ReturnValue ret = server.game().getPlayerByNameWildcard(popString(L), player);
	if(ret == RET_NOERROR)
		lua_pushnumber(L, env->addThing(player));
	else if(pushRet)
...
{
	//getPlayersByAccountId(accId)
	PlayerVector players = server.game().getPlayersByAccount(popNumber(L));
...
int32_t LuaScriptInterface::luaGetPlayersByIp(lua_State* L)
{
	...
	PlayerVector players = server.game().getPlayersByIP(popNumber(L), mask);
...
int32_t LuaScriptInterface::luaDoAddContainerItemEx(lua_State* L)
{
	...
	if(Container* container = env->getContainerByUID(popNumber(L)))
	{
		...
		ReturnValue ret = server.game().internalAddItem(nullptr, container, item);
		if(ret == RET_NOERROR)
...
int32_t LuaScriptInterface::luaDoAddContainerItem(lua_State* L)
{
	...
	while(itemCount > 0)
	{
		...
		ReturnValue ret = server.game().internalAddItem(nullptr, container, newItem.get());
		if(ret != RET_NOERROR)
		{
			lua_pushboolean(L, false);
			return 1;
...
int32_t LuaScriptInterface::luaDoCreatureSetLookDir(lua_State* L)
{
	...
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		...
		server.game().internalCreatureTurn(creature, dir);
		lua_pushboolean(L, true);
	}
	else
	{
...
int32_t LuaScriptInterface::luaDoCreatureSetSkullType(lua_State* L)
{
	...
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		...
		server.game().updateCreatureSkull(creature);
		lua_pushboolean(L, true);
	}
	else
	{
...
int32_t LuaScriptInterface::luaIsSightClear(lua_State* L)
{
	...
	lua_pushboolean(L, server.game().isSightClear(fromPos, toPos, floorCheck));
	return 1;
}
...
int32_t LuaScriptInterface::luaGetChannelUsers(lua_State* L)
{
	...
	if(ChatChannel* channel = server.chat().getChannelById(channelId))
	{
...
int32_t LuaScriptInterface::luaGetTownId(lua_State* L)
{
	...
	if(Town* town = server.towns().getTown(townName))
...
int32_t LuaScriptInterface::luaGetTownTemplePosition(lua_State* L)
{
	...
	if(lua_gettop(L) >= 2)
		popNumber(L);  // backward-compatibility: last argument was "displayError" true/false
...
int32_t LuaScriptInterface::luaGetSpectators(lua_State* L)
{
	...
	SpectatorList list;
	server.game().getSpectators(list, centerPos, false, multifloor, rangex, rangex, rangey, rangey);
	if(list.empty())
	{
		lua_pushnil(L);
		return 1;
	...
	ScriptEnviroment* env = getEnv();
	SpectatorList::const_iterator it = list.begin();
...
int32_t LuaScriptInterface::luaGetHighscoreString(lua_State* L)
{
	...
	if(skillId <= SKILL__LAST)
		lua_pushstring(L, server.game().getHighscoreString(skillId).c_str());
	else
		lua_pushboolean(L, false);
...
{
	//getWaypointList()
	WaypointMap waypointsMap = server.game().getMap()->getWaypoints().getWaypointsMap();
...
{
	//getGameState()
	lua_pushnumber(L, server.game().getGameState());
	return 1;
}
...
int32_t LuaScriptInterface::luaDoSetGameState(lua_State* L)
{
	...
	{
		server.dispatcher().addTask(Task::create(std::bind(&Game::setGameState, &server.game(), (GameState_t)id)));
		lua_pushboolean(L, true);
	}
	else
		lua_pushboolean(L, false);
...
int32_t LuaScriptInterface::luaDoCreatureExecuteTalkAction(lua_State* L)
{
	...
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
		lua_pushboolean(L, server.talkActions().onPlayerSay(creature, channelId, text, ignoreAccess));
	else
	{
		errorEx(getError(LUA_ERROR_CREATURE_NOT_FOUND));
		lua_pushboolean(L, false);
...
int32_t LuaScriptInterface::luaDoSaveServer(lua_State* L)
{
	...
	server.dispatcher().addTask(Task::create(std::bind(&Game::saveGameState, &server.game(), shallow)));
	return 1;
}
...
int32_t LuaScriptInterface::luaGetItemIdByName(lua_State* L)
{
	...
	uint16_t itemId = server.items().getKindIdByName(popString(L));
	if(itemId == 0)
	{
		if(displayError)
			errorEx(getError(LUA_ERROR_ITEM_NOT_FOUND));
...
int32_t LuaScriptInterface::luaGetItemInfo(lua_State* L)
{
	...
	ItemKindPC kind = server.items()[popNumber(L)];
	if (!kind) {
		lua_pushboolean(L, false);
		return 1;
	}
	...
	lua_newtable(L);
	setFieldBool(L, "stopTime", kind->stopTime);
	setFieldBool(L, "showCount", kind->showCount);
	setFieldBool(L, "clientCharges", kind->clientCharges);
	setFieldBool(L, "stackable", kind->stackable);
	setFieldBool(L, "showDuration", kind->showDuration);
	setFieldBool(L, "showCharges", kind->showCharges);
	setFieldBool(L, "showAttributes", kind->showCharges);
	setFieldBool(L, "distRead", kind->allowDistRead);
	setFieldBool(L, "readable", kind->canReadText);
	setFieldBool(L, "writable", kind->canWriteText);
	setFieldBool(L, "forceSerialize", kind->forceSerialize);
	setFieldBool(L, "vertical", kind->isVertical);
	setFieldBool(L, "horizontal", kind->isHorizontal);
	setFieldBool(L, "hangable", kind->isHangable);
	setFieldBool(L, "usable", kind->useable);
	setFieldBool(L, "movable", kind->moveable);
	setFieldBool(L, "pickupable", kind->pickupable);
	setFieldBool(L, "rotable", kind->rotable);
	setFieldBool(L, "replacable", kind->replaceable);
	setFieldBool(L, "hasHeight", kind->hasHeight);
	setFieldBool(L, "blockSolid", kind->blockSolid);
	setFieldBool(L, "blockPickupable", kind->blockPickupable);
	setFieldBool(L, "blockProjectile", kind->blockProjectile);
	setFieldBool(L, "blockPathing", kind->blockPathFind);
	setFieldBool(L, "allowPickupable", kind->allowPickupable);
	setFieldBool(L, "alwaysOnTop", kind->alwaysOnTop);

	createTable(L, "floorChange");
	for(int32_t i = CHANGE_FIRST; i <= CHANGE_LAST; ++i)
	{
		lua_pushnumber(L, i);
		lua_pushboolean(L, kind->hasFloorChange(static_cast<FloorChange_t>(i - 1)));
		pushTable(L);
	}

	pushTable(L);
	setField(L, "magicEffect", (int32_t)kind->magicEffect);
	setField(L, "fluidSource", (int32_t)kind->fluidSource);
	setField(L, "weaponType", (int32_t)kind->weaponType);
	setField(L, "bedPartnerDirection", (int32_t)kind->bedPartnerDir);
	setField(L, "ammoAction", (int32_t)kind->ammoAction);
	setField(L, "combatType", (int32_t)kind->combatType);
	setField(L, "corpseType", (int32_t)kind->corpseType);
	setField(L, "shootType", (int32_t)kind->shootType);
	setField(L, "ammoType", (int32_t)kind->ammoType);

	createTable(L, "transformUseTo");
	setField(L, "female", kind->transformUseTo[PLAYERSEX_FEMALE]);
	setField(L, "male", kind->transformUseTo[PLAYERSEX_MALE]);

	pushTable(L);
	setField(L, "transformToFree", kind->transformToFree);
	setField(L, "transformEquipTo", kind->transformEquipTo);
	setField(L, "transformDeEquipTo", kind->transformDeEquipTo);
	...
	setField(L, "attack", kind->attack);
	setField(L, "extraAttack", kind->extraAttack);
	setField(L, "defense", kind->defense);
	setField(L, "extraDefense", kind->extraDefense);
	setField(L, "armor", kind->armor);
	setField(L, "breakChance", kind->breakChance);
	setField(L, "hitChance", kind->hitChance);
	setField(L, "maxHitChance", kind->maxHitChance);
	setField(L, "runeLevel", kind->runeLevel);
	setField(L, "runeMagicLevel", kind->runeMagLevel);
	setField(L, "lightLevel", kind->lightLevel);
	setField(L, "lightColor", kind->lightColor);
	setField(L, "decayTo", kind->decayTo);
	setField(L, "rotateTo", kind->rotateTo);
	setField(L, "alwaysOnTopOrder", kind->alwaysOnTopOrder);
	setField(L, "shootRange", kind->shootRange);
	setField(L, "charges", kind->charges);
	setField(L, "decayTime", kind->decayTime);
	setField(L, "attackSpeed", kind->attackSpeed);
	setField(L, "wieldInfo", kind->wieldInfo);
	setField(L, "minRequiredLevel", kind->minReqLevel);
	setField(L, "minRequiredMagicLevel", kind->minReqMagicLevel);
	setField(L, "worth", kind->worth);
	setField(L, "levelDoor", kind->levelDoor);
	setField(L, "name", kind->name.c_str());
	setField(L, "plural", kind->pluralName.c_str());
	setField(L, "article", kind->article.c_str());
	setField(L, "description", kind->description.c_str());
	setField(L, "runeSpellName", kind->runeSpellName.c_str());
	setField(L, "vocationString", kind->vocationString.c_str());

	createTable(L, "abilities");
	setFieldBool(L, "manaShield", kind->abilities.manaShield);
	setFieldBool(L, "invisible", kind->abilities.invisible);
	setFieldBool(L, "regeneration", kind->abilities.regeneration);
	setFieldBool(L, "preventLoss", kind->abilities.preventLoss);
	setFieldBool(L, "preventDrop", kind->abilities.preventDrop);
	setField(L, "elementType", (int32_t)kind->abilities.elementType);
	setField(L, "elementDamage", kind->abilities.elementDamage);
	setField(L, "speed", kind->abilities.speed);
	setField(L, "healthGain", kind->abilities.healthGain);
	setField(L, "healthTicks", kind->abilities.healthTicks);
	setField(L, "manaGain", kind->abilities.manaGain);
	setField(L, "manaTicks", kind->abilities.manaTicks);
	setField(L, "conditionSuppressions", kind->abilities.conditionSuppressions);

	//TODO: absorb, increment, reflect, skills, skillsPercent, stats, statsPercent

	pushTable(L);
	setField(L, "group", (int32_t)kind->group);
	setField(L, "type", (int32_t)kind->type);
	setFieldFloat(L, "weight", kind->weight);
	return 1;
}
...
int32_t LuaScriptInterface::luaDoAddIpBanishment(lua_State* L)
{
	...
	int64_t length = time(nullptr) + server.configManager().getNumber(ConfigManager::IPBANISHMENT_LENGTH);
...
int32_t LuaScriptInterface::luaDoAddAccountBanishment(lua_State* L)
{
	...
	int64_t length = time(nullptr) + server.configManager().getNumber(ConfigManager::BAN_LENGTH);
	std::string statement, comment;
...
int32_t LuaScriptInterface::luaGetExperienceStage(lua_State* L)
{
	...
	lua_pushnumber(L, server.game().getExperienceStage(popNumber(L), divider));
	return 1;
}
...
{
	//getDataDir()
	lua_pushstring(L, getFilePath(FileType::OTHER, "").c_str());
	return 1;
}
...
{
	//getLogsDir()
	lua_pushstring(L, getFilePath(FileType::LOG, "").c_str());
	return 1;
}
...
{
	//getConfigFile()
	lua_pushstring(L, server.configManager().getString(ConfigManager::CONFIG_FILE).c_str());
	return 1;
}
...
{
	//getConfigValue(key)
	server.configManager().getValue(popString(L), L);
	return 1;
}
...
int32_t LuaScriptInterface::luaStdCout(lua_State* L) {
	StringVector data;
	for(int32_t i = 0, params = lua_gettop(L); i < params; ++i)
		data.push_back(popString(L));

	for(StringVector::reverse_iterator it = data.rbegin(); it != data.rend(); ++it)
		LOGi(*it);
...
int32_t LuaScriptInterface::luaDatabaseExecute(lua_State* L)
{
	...
	lua_pushboolean(L, server.database().executeQuery(popString(L)));
	return 1;
}
...
int32_t LuaScriptInterface::luaDatabaseEscapeString(lua_State* L)
{
	//db.escapeString(str)
	DBQuery query; //lock mutex
	lua_pushstring(L, server.database().escapeString(popString(L)).c_str());
	return 1;
}
...
int32_t LuaScriptInterface::luaDatabaseEscapeBlob(lua_State* L)
{
	...
	lua_pushstring(L, server.database().escapeBlob(popString(L).c_str(), length).c_str());
	return 1;
}
...
int32_t LuaScriptInterface::luaDatabaseLastInsertId(lua_State* L)
{
	//db.lastInsertId()
	DBQuery query; //lock mutex
	lua_pushnumber(L, server.database().getLastInsertId());
	return 1;
}
...
{
	//db.stringComparison()
	lua_pushstring(L, server.database().getStringComparison().c_str());
	return 1;
}
...
{
	//db.updateLimiter()
	lua_pushstring(L, server.database().getUpdateLimiter().c_str());
	return 1;
}
