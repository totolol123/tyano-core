...
void IOLoginData::loadCharacters(Account& account) {
	...
	do {
		auto character = Account::CharacterP(new Account::Character);
		character->setName(result->getDataString("name"));

		uint32_t level = result->getDataInt("level");
		Vocation* vocation = Vocations::getInstance()->getVocation(result->getDataInt("vocation"));

		std::ostringstream type;
		type << level << ", " << vocation->getName();
		character->setType(type.str());
		...
		if (GameServer* server = GameServers::getInstance()->getServerById(result->getDataInt("world_id"))) {
			...
			LOGe("[IOLoginData::loadAccount] Invalid server for player '" << character.getName() << "'.");
			continue;
		}
#endif
...
const Group* IOLoginData::getPlayerGroupByAccount(uint32_t accountId)
{
	...
	return Groups::getInstance()->getGroup(result->getDataInt("group_id"));
}
...
bool IOLoginData::loadPlayer(Player* player, const std::string& name, bool preLoad /*= false*/)
{
	...
	query << server.configManager().getNumber(ConfigManager::WORLD_ID) << " AND `deleted` = 0 LIMIT 1";
	...
	if(server.configManager().getBool(ConfigManager::STORE_DIRECTION))
	...
	if(Town* town = server.towns().getTown(player->town))
	...
	else if(server.configManager().getBool(ConfigManager::INGAME_GUILD_MANAGEMENT))
	{
		query.str("");
	...
	if((result = db.storeQuery(query.str())))
	{
		loadItems(itemMap, std::move(result));

		for(ItemMap::reverse_iterator rit = itemMap.rbegin(); rit != itemMap.rend(); ++rit)
		{
			const boost::intrusive_ptr<Item>& item = rit->second.first;
			int32_t pid = rit->second.second;
			if(pid > 0 && pid < 11)
				player->__internalAddThing(pid, item.get());
			else
			{
				it = itemMap.find(pid);
				if(it != itemMap.end())
			...
			if(pid >= 0 && pid < 100)
			{
				if(Container* c = item->getContainer())
				{
					if(Depot* depot = c->getDepot())
						...
						LOGe("[IOLoginData::loadPlayer] Cannot load depot " << pid << " for player " << name);
				}
				else
					LOGe("[IOLoginData::loadPlayer] Cannot load depot " << pid << " for player " << name);
			}
...
void IOLoginData::loadItems(ItemMap& itemMap, DBResultP result)
{
	do
	{
		uint64_t attrSize = 0;
		...
		if(boost::intrusive_ptr<Item> item = Item::CreateItem(result->getDataInt("itemtype"), result->getDataInt("count")))
		{
			if(!item->unserializeAttr(propStream))
				LOGe("[IOLoginData::loadItems] Unserialize error for item with id " << item->getId());
...
bool IOLoginData::savePlayer(Player* player, bool preSave/* = true*/, bool shallow/* = false*/)
{
	if(preSave && player->health <= 0)
	{
		...
		{
			player->health = server.configManager().getNumber(ConfigManager::BLACK_SKULL_DEATH_HEALTH);
			player->mana = server.configManager().getNumber(ConfigManager::BLACK_SKULL_DEATH_MANA);
		}
		else
		{
	...
	{
		if(Item* item = player->inventory[slotId].get())
	...
	for(DepotMap::iterator it = player->depots.begin(); it != player->depots.end(); ++it)
	{
		/*if(it->second.second)
		{
			...
			ss << it->first << ",";*/
			itemList.push_back(itemBlock(it->first, it->second.first.get()));
		//}
	}
...
bool IOLoginData::playerDeath(Player* player, const DeathList& dl)
{
	...
	query << "INSERT INTO `player_deaths` (`player_id`, `date`, `level`) VALUES (" << player->getGUID()
		<< ", " << time(nullptr) << ", " << player->getLevel() << ")";
	if(!db.executeQuery(query.str()))
		return false;

	int32_t i = 0, size = dl.size(), tmp = server.configManager().getNumber(ConfigManager::DEATH_ASSISTS) + 1;
	if(tmp > 0 && size > tmp)
		size = tmp;
...
bool IOLoginData::getGuidByName(uint32_t& guid, const std::string& name, bool multiworld /*= false*/) {
	std::string resultName = name;
	return getGuidByName(guid, resultName, multiworld);
}
...
bool IOLoginData::getGuidByNameEx(uint32_t& guid, bool &specialVip, std::string& name)
{
	Database& db = server.database();
	DBQuery query;
	query << "SELECT `id`, `name`, `group_id` FROM `players` WHERE `name` " << db.getStringComparison() << db.escapeString(name) << " AND `deleted` = 0 AND `world_id` = " << server.configManager().getNumber(ConfigManager::WORLD_ID) << " LIMIT 1";
...
bool IOLoginData::changeName(uint32_t guid, std::string newName, std::string oldName)
{
	...
	query << "INSERT INTO `player_namelocks` (`player_id`, `name`, `new_name`, `date`) VALUES ("<< guid << ", "
		<< db.escapeString(oldName) << ", " << db.escapeString(newName) << ", " << time(nullptr) << ")";
	if(!db.executeQuery(query.str()))
		return false;
...
bool IOLoginData::createCharacter(uint32_t accountId, std::string characterName, int32_t vocationId, uint16_t sex)
{
	...
	uint32_t level = server.configManager().getNumber(ConfigManager::START_LEVEL), tmpLevel = std::min((uint32_t)7, (level - 1));
	uint64_t exp = 0;
	if(level > 1)
...
{
	if(server.game().getPlayerByName(characterName))
...
uint32_t IOLoginData::getAccountIdByName(const std::string& name) const
{
	...
	return result->getDataInt("account_id");
}
