...
class LuaScriptInterface
{
	public:
		...
		bool call(const std::string& script, std::string& result);
		bool loadBuffer(const std::string& text, Npc* npc = nullptr);
		bool loadFile(const std::string& file, Npc* npc = nullptr);
		bool loadDirectory(const std::string& dir, Npc* npc = nullptr);
		...
		static void getValue(const std::string& key, lua_State* L, lua_State* luaState);
