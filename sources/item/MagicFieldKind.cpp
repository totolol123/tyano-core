...
LOGGER_DEFINITION(ts::item::MagicFieldKind);


MagicFieldKind::MagicFieldKind(const ClassPC& clazz, KindId id)
...
bool MagicFieldKind::assemble() {
	if (!Kind::assemble()) {
		return false;
	}

	if (_damage != 0 || _damageInterval != Duration::zero() || _damageCount != 0 || _damageType != DamageType::NONE || _totalDamage != 0) {
		if (_damage == 0) {
		...
		if (_damageInterval == Duration::zero()) {
			if (_damageCount > 1) {
...
bool MagicFieldKind::setParameter(const std::string& name, const std::string& value, xmlNodePtr deprecatedNode) {
	...
	if (name == "damage") {
		...
		_damageInterval = std::chrono::milliseconds(numericValue);
	}
		...
		for (xmlNodePtr child = deprecatedNode->children; child != nullptr; child = child->next) {
			...
			if (name == "count") {
				...
				_damageInterval = std::chrono::milliseconds(interval);
				continue;
			}
			else if (name == "start") {
				if (!(value >> start)) {
...
template<>
ClassDescriptor<MagicFieldKind>::DynamicAttributesP ClassDescriptor<MagicFieldKind>::getDynamicAttributes() {
	return Base::getDynamicAttributes();
}


template<>
std::string ClassDescriptor<MagicFieldKind>::getId() {
	return "magicField";
}


template<>
std::string ClassDescriptor<MagicFieldKind>::getName() {
	return "Magic Field";
}
